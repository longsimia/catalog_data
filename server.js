...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderHighlightedHistoryHtml(sourceText,targetText){const left=splitHistoryParagraphs(sourceText);const right=splitHistoryParagraphs(targetText);const{leftMatched}=buildLcsMatchedIndices(left,right);const parts=[];let collapsed=false;for(let idx=0;idx<left.length;idx+=1){const line=left[idx]||'';if(leftMatched.has(idx)){if(String(line).trim()){if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}}else if(parts.length&&parts[parts.length-1]!==''&&parts[parts.length-1]!=='<span class="history-ellipsis">…</span>'){parts.push('');}continue;}collapsed=false;if(!String(line).trim()){parts.push('');continue;}parts.push('<span class="diff-line"><mark class="diff-mark">'+escapeHtml(line)+'</mark></span>');}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}
