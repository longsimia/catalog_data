...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderHighlightedHistoryHtml(sourceText,targetText){const left=splitHistoryCompareBlocks(sourceText);const right=splitHistoryCompareBlocks(targetText);const{leftMatched,rightMatched}=buildLcsMatchedIndices(left,right);const parts=[];let collapsed=false;let rightCursor=0;for(let idx=0;idx<left.length;idx+=1){if(leftMatched.has(idx)){while(rightCursor<right.length&&!rightMatched.has(rightCursor))rightCursor+=1;rightCursor+=1;if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}continue;}collapsed=false;let compareBlock='';while(rightCursor<right.length&&!rightMatched.has(rightCursor)){compareBlock=right[rightCursor]||'';rightCursor+=1;break;}parts.push(renderHistoryBlock(left[idx],getHistoryBlockChangeSign(left[idx],compareBlock)));}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}
    function buildHistoryCompareContext(sourceText,targetText){const left=splitHistoryCompareBlocks(sourceText);const right=splitHistoryCompareBlocks(targetText);const{leftMatched,rightMatched}=buildLcsMatchedIndices(left,right);const leftVisible=new Set(),rightVisible=new Set();for(let idx=0;idx<left.length;idx+=1){if(leftMatched.has(idx))continue;for(let pointer=Math.max(0,idx-1);pointer<=Math.min(left.length-1,idx+1);pointer+=1)leftVisible.add(pointer);}for(let idx=0;idx<right.length;idx+=1){if(rightMatched.has(idx))continue;for(let pointer=Math.max(0,idx-1);pointer<=Math.min(right.length-1,idx+1);pointer+=1)rightVisible.add(pointer);}if(!leftVisible.size&&!rightVisible.size){return{leftHtml:'<div class="diff-empty">此版本與目前內文相同。</div>',rightHtml:'<div class="diff-empty">此版本與目前內文相同。</div>'};}function renderBlocks(blocks,visible,matched){const parts=[];let collapsed=false;for(let idx=0;idx<blocks.length;idx+=1){if(!visible.has(idx)){if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}continue;}collapsed=false;const block=blocks[idx]||'';const highlighted=!matched.has(idx);parts.push(highlighted?'<span class="diff-line"><mark class="diff-mark">'+escapeHtml(block||' ')+'</mark></span>':'<span class="diff-line">'+escapeHtml(block||' ')+'</span>');}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}return{leftHtml:renderBlocks(left,leftVisible,leftMatched),rightHtml:renderBlocks(right,rightVisible,rightMatched)};}
