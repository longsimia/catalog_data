...
function getDocxRunStyle(runXml) {
  ...
  const fontHint = ['新細明體', '宋体', 'MS PGothic'].includes(fontHintRaw) ? 'Georgia' : fontHintRaw;
...
function normalizeTextTocEntry(entry, fallbackKind = 'manual') {
  if (!entry || typeof entry !== 'object' || typeof entry.label !== 'string') return null;
  const kind = entry.kind === 'auto' || entry.kind === 'hidden' || entry.kind === 'manual'
    ? entry.kind
    : fallbackKind;
  return {
    id: String(entry.id || `${kind}-${Date.now()}`).slice(0, 200),
    kind,
    label: String(entry.label || '未命名項目').slice(0, 500),
    start: Math.max(0, Number(entry.start) || 0),
    end: Math.max(0, Number(entry.end) || 0),
    sourceIndex: Math.max(0, Number(entry.sourceIndex) || 0),
    lineCount: Math.max(1, Number(entry.lineCount) || 1),
    text: String(entry.text || '').slice(0, TEXT_TOC_MAX_TEXT_LENGTH),
    anchorBefore: String(entry.anchorBefore || '').slice(0, 500),
    anchorAfter: String(entry.anchorAfter || '').slice(0, 500),
    removed: !!entry.removed
  };
}

function normalizeTextTocState(state) {
  if (!state || typeof state !== 'object') return null;
  if (!Array.isArray(state.manualTocEntries) && !Array.isArray(state.tocEntries)) return null;
  const manualTocEntries = (Array.isArray(state.manualTocEntries) ? state.manualTocEntries : [])
    .slice(0, TEXT_TOC_MAX_ENTRIES)
    .map(entry => normalizeTextTocEntry(entry, entry?.kind === 'hidden' ? 'hidden' : 'manual'))
    .filter(Boolean);
  const tocEntries = (Array.isArray(state.tocEntries) ? state.tocEntries : [])
    .slice(0, TEXT_TOC_MAX_ENTRIES)
    .map(entry => normalizeTextTocEntry(entry, entry?.kind === 'auto' ? 'auto' : 'manual'))
    .filter(Boolean);
  const savedAt = Number(state.savedAt);
  return {
    manualTocEntries,
    tocEntries,
    activeTocEntryId: typeof state.activeTocEntryId === 'string' ? state.activeTocEntryId.slice(0, 200) : '',
    tocOrderIds: (Array.isArray(state.tocOrderIds) ? state.tocOrderIds : [])
      .slice(0, TEXT_TOC_MAX_ENTRIES * 2)
      .map(id => String(id || '').slice(0, 200))
      .filter(Boolean),
    savedAt: Number.isFinite(savedAt) && savedAt > 0 ? savedAt : Date.now(),
    savedById: typeof state.savedById === 'string' ? state.savedById.slice(0, 200) : '',
    savedBy: typeof state.savedBy === 'string' ? state.savedBy.slice(0, 500) : ''
  };
}

function readTextHistoryData(collection, itemId, fileKey) {
  const historyPath = getTextHistoryPath(collection, itemId, fileKey);
  if (!fs.existsSync(historyPath)) return { versions: [], tocState: null };
  const raw = readJSON(historyPath, {});
  return {
    versions: Array.isArray(raw?.versions) ? raw.versions : [],
    tocState: normalizeTextTocState(raw?.tocState)
  };
}

function writeTextHistoryData(collection, itemId, fileKey, data = {}) {
  const historyPath = getTextHistoryPath(collection, itemId, fileKey);
  const versions = Array.isArray(data?.versions) ? data.versions : [];
  const tocState = normalizeTextTocState(data?.tocState);
  if (!versions.length && !tocState) {
    if (fs.existsSync(historyPath)) fs.unlinkSync(historyPath);
    return;
  }
  fs.mkdirSync(path.dirname(historyPath), { recursive: true });
  writeJSON(historyPath, { versions, ...(tocState ? { tocState } : {}) });
}

function readTextHistoryVersions(collection, itemId, fileKey) {
  return readTextHistoryData(collection, itemId, fileKey).versions;
}

function writeTextHistoryVersions(collection, itemId, fileKey, versions) {
  const data = readTextHistoryData(collection, itemId, fileKey);
  writeTextHistoryData(collection, itemId, fileKey, { ...data, versions });
}

function readTextTocState(collection, itemId, fileKey) {
  return readTextHistoryData(collection, itemId, fileKey).tocState;
}

function writeTextTocState(collection, itemId, fileKey, state) {
  const tocState = normalizeTextTocState(state);
  if (!tocState) return null;
  const data = readTextHistoryData(collection, itemId, fileKey);
  writeTextHistoryData(collection, itemId, fileKey, { ...data, tocState });
  return tocState;
}

function appendTextHistoryVersion(collection, item, fileKey, entry) {
  const data = readTextHistoryData(collection, item?.id, fileKey);
  const versions = data.versions;
  const last = versions.length ? versions[versions.length - 1] : null;
  const tocState = normalizeTextTocState(entry.tocState) || data.tocState;
  const nextEntry = {
  ...
  if (
    ...
    if (tocState) last.tocState = tocState;
    writeTextHistoryData(collection, item?.id, fileKey, { versions, tocState });
    return versions;
  }
...
function moveTextHistoryVersions(collection, itemId, fromFileKey, toFileKey) {
  ...
  const data = readTextHistoryData(collection, itemId, fromFileKey);
  writeTextHistoryData(collection, itemId, toFileKey, data);
...
function pruneExpiredTextHistoryFiles(now = Date.now()) {
  ...
  for (const historyPath of listTextHistoryFiles()) {
    ...
    const tocState = normalizeTextTocState(raw?.tocState);
    if (!versions.length) {
      if (tocState) continue;
    ...
    try {
      if (tocState) writeJSON(historyPath, { versions: [], tocState });
      else fs.unlinkSync(historyPath);
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .topbar{display:flex;align-items:center;justify-content:space-between;min-height:30px;gap:16px;margin-bottom:16px}
...
function renderTextPreviewPage(item, file, text, options = {}) {
  ...
  const inlineTocStateJson = JSON.stringify(options.tocState || null)
    .replace(/</g, '\\u003c')
    .replace(/>/g, '\\u003e')
    .replace(/&/g, '\\u0026');
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .editor{display:block;width:100%;min-height:0;border:none;outline:none;resize:none;overflow:hidden;background:transparent;color:inherit;padding:0;field-sizing:content}
    .editor[hidden]{display:none}
    .editor-host{display:block;width:100%;min-height:1.92em;margin:0;color:inherit;font:inherit;font-size:16px;line-height:1.92;letter-spacing:.01em}
    .editor-host[hidden]{display:none}
    .editor-host .cm-editor{width:100%;min-height:1.92em;background:transparent;color:inherit;font:inherit;font-family:inherit;font-size:16px;line-height:1.92;letter-spacing:.01em}
    ...
    .toc-window{width:min(88vw,336px)}
    ...
    .divider{height:1px;background:var(--line);margin:0 0 36px}
    .txt-editable-page.is-stuck .meta-side-editable{position:fixed;inset:0 0 auto;z-index:50;min-height:50px;padding:calc(10px + env(safe-area-inset-top)) max(24px,calc((100vw - 760px)/2 + 24px)) 10px;background:var(--bg);align-items:stretch}
    .txt-editable-page.is-stuck .editable-actions{width:100%;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;white-space:nowrap;scrollbar-width:none}
    .txt-editable-page.is-stuck .editable-actions::-webkit-scrollbar{display:none}
    .txt-editable-page.is-stuck>.divider{position:fixed;inset:calc(50px + env(safe-area-inset-top)) 0 auto;z-index:49;margin:0}
    .txt-editable-page.is-stuck>.article{padding-top:37px}
    .footer{margin-top:44px;padding-top:14px;border-top:1px solid var(--line);font-size:14px;color:var(--muted);text-align:center}
    .article blockquote{margin:1.8em 0;padding-left:18px;border-left:3px solid #d7d7d7;color:#444}
    html[data-theme="dark"] .article blockquote{border-left-color:#8f8679;color:#d2cbc2}
    ...
    @media (max-width: 720px){.page{padding:40px 20px 48px}.meta-head{gap:12px;flex-direction:column}.meta-side{width:100%;align-items:flex-start;gap:12px}.actions{width:100%;justify-content:flex-start}.encoding-field{flex-wrap:wrap;align-items:flex-start}.encoding-toggle,.encoding-menu-item{font-size:12px}.article,.article-body,.editor{font-size:16px;line-height:1.88}.format-grid{grid-template-columns:max-content}.format-action-btn-wide{grid-column:auto}}
    @media (max-width: 720px){.txt-editable-page.is-stuck .meta-head::after{content:"";display:block;height:50px}.txt-editable-page.is-stuck .meta-side-editable{padding-right:20px;padding-left:20px}.txt-editable-page.is-stuck .editable-actions{justify-content:flex-start}}
  </style>
  <script>
    (() => {
      const mode = localStorage.getItem('theme-mode') === 'dark' ? 'dark' : 'light';
...
<body>
  <main class="page${isTxt && canEditTxt ? ' txt-editable-page' : ''}">
    <header class="meta">
      <div class="meta-head">
        <div class="meta-main">
  ...
  ${isTxt ? `<div class="modal floating-modal" id="tocModal" aria-hidden="true"><div class="modal-bg" id="tocModalBg"></div><section class="floating-window toc-window" id="tocWindow" role="dialog" aria-labelledby="tocModalTitle"><div class="window-head" id="tocDragHandle"><h3 id="tocModalTitle">目錄</h3><button type="button" class="window-close" id="tocCloseBtn" aria-label="關閉">×</button></div><div class="window-body"><div id="tocEmpty" class="toc-empty" style="display:none">目前沒有可跳轉的目錄項目。</div><div id="tocList" class="toc-list"></div><div class="modal-actions toc-actions" style="margin-top:14px"><button type="button" id="tocClearBtn">清空</button><button type="button" id="tocRefreshBtn">掃描</button>${canEditTxt ? `<button type="button" id="tocAddBtn">新增目前段落</button>` : ''}</div></div></section></div>` : ''}
  ${isTxt && canEditTxt ? `<div class="modal floating-modal" id="formatModal" aria-hidden="true"><div class="modal-bg" id="formatModalBg"></div><section class="floating-window format-modal-card" id="formatWindow" role="dialog" aria-labelledby="formatModalTitle"><div class="window-head" id="formatDragHandle"><h3 id="formatModalTitle">格式化排版</h3><button type="button" class="window-close" id="formatCloseBtn" aria-label="關閉">×</button></div><div class="window-body"><div class="encoding-field"><label for="encodingToggleBtn">切換編碼</label><div class="encoding-menu-wrap"><button type="button" id="encodingToggleBtn" class="encoding-toggle" aria-haspopup="true" aria-expanded="false" aria-controls="encodingMenu"><span id="encodingToggleLabel">${escapeXml(getEncodingToggleLabel(selectedEncoding))}</span><span class="encoding-toggle-caret" aria-hidden="true">▾</span></button><select id="encodingSelect" class="encoding-select" aria-label="TXT 編碼" tabindex="-1">${encodingOptionsHtml}</select><div id="encodingMenu" class="encoding-menu" role="menu">${encodingMenuHtml}</div></div></div><div class="format-grid"><button type="button" class="format-action-btn format-action-btn-wide" data-action="open_toc">目錄</button><button type="button" class="format-action-btn format-action-btn-wide" data-action="find_replace">尋找與取代</button><button type="button" class="format-action-btn" data-action="indent_add">插入段首縮排</button><button type="button" class="format-action-btn" data-action="indent_remove">去除段首縮排</button><button type="button" class="format-action-btn" data-action="add_blank_line">在段落與段落間新增一個空行</button><button type="button" class="format-action-btn" data-action="trim_blank_lines">去除段落與段落間所有的空行</button><button type="button" class="format-action-btn" data-action="punct_fullwidth">半形符號轉全形符號</button><button type="button" class="format-action-btn" data-action="cn_to_twp">简体中文 轉 繁體中文</button><button type="button" class="format-action-btn" data-action="t_to_cn">繁體中文 轉 简体中文</button><button type="button" class="format-action-btn" data-action="cjk_spacing">在中英文之間插入空白</button></div></div></section></div>` : ''}
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function syncStickyActions(){stickyFrame=0;if(!previewPage||!filenameInput)return;previewPage.classList.toggle('is-stuck',filenameInput.getBoundingClientRect().bottom<=0);}
    function scheduleStickySync(){if(stickyFrame)return;stickyFrame=window.requestAnimationFrame(syncStickyActions);}
    function snapshotState(){return{text:editor?editor.value:'',filename:filenameInput?filenameInput.value:'',selectionStart:editor?.selectionStart??0,selectionEnd:editor?.selectionEnd??0};}
    function statesEqual(a,b){return(a?.text||'')===(b?.text||'')&&(a?.filename||'')===(b?.filename||'');}
    function computeUnsavedChanges(){return (!!editor&&editor.value!==lastSavedText)||(!!filenameInput&&filenameInput.value!==lastSavedFilename);}
    function hasUnsavedChanges(){return dirtyState;}
    function markDirtyState(){dirtyState=true;if(!status||status.dataset.state==='error')return;if(status.textContent!=='尚有未儲存的變更。'||status.dataset.state)setStatusMessage('尚有未儲存的變更。');}
    function syncDirtyState(){dirtyState=computeUnsavedChanges();if(!status||status.dataset.state==='error')return;if(dirtyState)setStatusMessage('尚有未儲存的變更。');else if(status.textContent==='尚有未儲存的變更。')setStatusMessage('');}
    ...
    function commitPendingInputGroup(){clearInputGroupTimer();if(!inputGroupState)return;rememberUndoState(inputGroupState);inputGroupState=null;inputGroupTarget='';syncDirtyState();}
    ...
    function applyState(state,{focusEditor=true}={}){clearInputGroupTimer();inputGroupState=null;inputGroupTarget='';pendingInputState=null;suspendTracking=true;if(editor){editor.value=state?.text||'';const start=Number(state?.selectionStart),end=Number(state?.selectionEnd);if(Number.isFinite(start)&&Number.isFinite(end))editor.setSelectionRange(start,end,'forward');}if(filenameInput)filenameInput.value=state?.filename||'';resizeEditor();suspendTracking=false;syncDirtyState();updateUndoButtons();updateFindSummary();if(focusEditor)editor?.focus();}
    function applyTrackedState(nextState,successMessage,unchangedMessage='目前內容沒有變化。'){commitPendingInputGroup();const before=snapshotState();if(statesEqual(before,nextState)){setStatusMessage(unchangedMessage);return false;}rememberUndoState(before,true);applyState(nextState);setStatusMessage(successMessage,'success');return true;}
    ...
    function handleEditorInput(){if(suspendTracking)return;if(!isComposing&&pendingInputState){const current=snapshotState();if(!statesEqual(pendingInputState,current))rememberUndoState(pendingInputState);}pendingInputState=null;if(inputGroupState)scheduleInputGroupCommit();resizeEditor();markDirtyState();if(findReplaceModal?.classList.contains('on'))updateFindSummary();}
    function handleFilenameInput(){if(suspendTracking)return;if(!isComposing&&pendingInputState){const current=snapshotState();if(!statesEqual(pendingInputState,current))rememberUndoState(pendingInputState);}pendingInputState=null;if(inputGroupState)scheduleInputGroupCommit();markDirtyState();}
    ...
    function addBlankLineBetweenParagraphs(text){return normalizeLineEndings(text).split('\\n').join('\\n\\n');}
    ...
    async function openHistoryModal(){await loadHistoryVersions();openModal(historyModal);historyCloseBtn?.focus();}
    function renderHistoryButton(){if(!historySlot)return;historySlot.innerHTML='<button type="button" class="action-btn" id="historyBtn" title="編輯記錄">編輯記錄</button>';document.getElementById('historyBtn')?.addEventListener('click',openHistoryModal);}
    ...
    function insertIndentAtCursor(event){if(!editor||event.key!=='Tab'||event.shiftKey||event.altKey||event.ctrlKey||event.metaKey)return false;event.preventDefault();commitPendingInputGroup();rememberUndoState(snapshotState(),true);const start=editor.selectionStart??0,end=editor.selectionEnd??start;editor.setRangeText(INDENT,start,end,'end');resizeEditor();syncDirtyState();return true;}
    function prepareEnterInput(event){if(!editor||event.key!=='Enter'||event.altKey||event.ctrlKey||event.metaKey)return;if(isComposing||event.isComposing||editor.readOnly)return;commitPendingInputGroup();pendingInputState=snapshotState();}
    ...
    syncStickyActions();renderHistoryButton();updateUndoButtons();loadHistoryVersions();window.setInterval(()=>{if(hasUnsavedChanges())saveText('auto');},AUTOSAVE_MS);window.addEventListener('beforeunload',handleBeforeUnload);history.pushState({txtPreviewGuard:true},'');window.addEventListener('popstate',()=>{if(allowLeave||!hasUnsavedChanges())return;history.pushState({txtPreviewGuard:true},'');showLeaveModal(true);});
  </script>` : ''}
  ${noContextMenuScript}
</body>
</html>`;
...
async function sendResolvedPreview(res, item, preview, previewIndex, options = {}) {
  ...
  const tocMemoryKey = `${sanitizeCollectionKey(collection)}:${String(item?.id || '')}:${String(preview.file?.key || preview.file?.relativePath || preview.filename || preview.file?.name || preview.type || 'preview')}`;
  ...
  const previewTocPath = canEditTxt ? withCollection(`/api/preview/${encodeURIComponent(item.id)}/${previewIndex}/toc`, collection) : '';
  const tocState = preview.file?.ext === '.txt'
    ? readTextTocState(collection, item?.id, preview.file.key)
    : null;
...
app.put('/api/preview/:id/:index', auth, (req, res) => {
  try {
    ...
    const hasSubmittedTocState = Object.prototype.hasOwnProperty.call(req.body || {}, 'tocState');
    const submittedTocState = hasSubmittedTocState ? normalizeTextTocState(req.body?.tocState) : null;
    if (hasSubmittedTocState && !submittedTocState) {
      return res.status(400).json({ error: '目錄資料格式不正確。' });
    }
    ...
    const currentTocState = submittedTocState
      ? {
          ...submittedTocState,
      ...
      : readTextTocState(collection, item?.id, renamedFile.key);
...
app.put('/api/preview/:id/:index/toc', auth, (req, res) => {
  try {
    const collection = getC(req);
    if (!canEditTxtPreview(req.authUser, collection)) return res.status(403).json({ error: '你沒有編輯 TXT 的權限。' });
    const collectionDenied = ensureCollectionAccessOrNull(collection, req.authUser?.role, readCfg());
    ...
    if (!canAccessItemByRole(item, req.authUser?.role)) return res.status(403).json({ error: '你沒有權限存取這個項目。' });
    const file = getPreviewableFiles(item, collection)[Number(req.params.index) || 0];
    if (!file || file.ext !== '.txt') return res.status(415).json({ error: '目前只能儲存 TXT 的目錄。' });
    const normalizedTocState = normalizeTextTocState(req.body?.tocState);
    if (!normalizedTocState) return res.status(400).json({ error: '目錄資料格式不正確。' });
    const tocState = writeTextTocState(collection, item.id, file.key, {
      ...normalizedTocState,
      savedAt: Date.now(),
      savedById: req.authUser?.id || '',
      savedBy: req.authUser?.username || ''
    });
    return res.json({ ok: true, tocState });
  } catch (e) {
    return res.status(500).json({ error: e.message });
  }
});
