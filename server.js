...
<body>
  ...
  ${scrollMemoryScript}
  ${isTxt ? `<script>(()=>{const tocBtn=document.getElementById('tocBtn'),tocModal=document.getElementById('tocModal'),tocModalBg=document.getElementById('tocModalBg'),tocCloseBtn=document.getElementById('tocCloseBtn'),tocList=document.getElementById('tocList'),tocEmpty=document.getElementById('tocEmpty'),tocRefreshBtn=document.getElementById('tocRefreshBtn'),tocAddBtn=document.getElementById('tocAddBtn'),tocWindow=document.getElementById('tocWindow'),tocDragHandle=document.getElementById('tocDragHandle'),editor=document.getElementById('editor'),txtBody=document.getElementById('txtBody');let tocEntries=[],activeTocEntryId='',manualTocEntries=[];function normalizeTocText(text){return String(text||'').replace(/\\r\\n/g,'\\n').replace(/\\r/g,'\\n');}function buildRawTocEntries(text){const normalized=normalizeTocText(text);const lines=normalized.split('\\n');const entries=[];let offset=0,start=-1,entryLines=[],end=-1;function pushEntry(){if(start<0||!entryLines.length)return;const firstLine=(entryLines.find(line=>line.trim())||'').replace(/\\s+/g,' ').trim();entries.push({sourceIndex:entries.length,start,end,label:firstLine||('第 '+(entries.length+1)+' 段'),lineCount:entryLines.length,text:entryLines.join('\\n')});start=-1;entryLines=[];end=-1;}for(let idx=0;idx<lines.length;idx+=1){const line=lines[idx],lineStart=offset,hasBreak=idx<lines.length-1;offset+=line.length+(hasBreak?1:0);if(line.trim()){if(start<0)start=lineStart;entryLines.push(line);end=offset-(hasBreak?1:0);continue;}pushEntry();}pushEntry();return entries;}function isExcludedLabel(label){const text=String(label||'').trim();if(!text)return true;if(/^[【《「〔\\(（][^【《「〔\\(（\\]】》」〕\\)）]*[】》」〕\\)）]$/.test(text))return true;if(/^[\\s　]*[【《「〔\\(（]/.test(text))return true;return false;}function isSceneLikeLabel(label){const text=String(label||'').trim();if(!text||isExcludedLabel(text))return false;const solidSymRe=/^[\\s　]*[■□●○◆◇★☆▲△▼▽▶▷◀◁◉◈◦•※✦✧✩✪✫✬✭✮✯✰◐◑◒◓◔◕⬛⬜🔲🔳▪▫◎〇]/u;const dateRe=/\\d{1,2}[\\s　]*[月\\/\\-.]\\s*\\d{1,2}|\\d{4}[\\/\\-.]\\d{1,2}[\\/\\-.]\\d{1,2}|星期[一二三四五六七日天]|週[一二三四五六日天]|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|Mon\\.?|Tue\\.?|Wed\\.?|Thu\\.?|Fri\\.?|Sat\\.?|Sun\\.?|(?:^|\\s)(?:[AP]M)(?:\\s|$)|午前|午後|深夜|早朝|夜明け/iu;return solidSymRe.test(text)||dateRe.test(text);}function buildAutoTocEntries(text){const rawEntries=buildRawTocEntries(text);const strictEntries=rawEntries.filter(entry=>isSceneLikeLabel(entry.label));const fallbackEntries=rawEntries.filter(entry=>entry.label.replace(/[\\s　]/g,'').length>=1);const picked=(strictEntries.length?strictEntries:fallbackEntries).filter(entry=>!isExcludedLabel(entry.label)||!strictEntries.length);return picked.map((entry,listIndex)=>({...entry,id:'auto-'+entry.sourceIndex,kind:'auto',displayOrder:listIndex}));}function getSourceText(){if(editor)return editor.value||'';if(txtBody)return Array.from(txtBody.querySelectorAll('[data-block-index]')).map(node=>node.textContent||'').join('\\n\\n');return '';}function getRawEntries(){return buildRawTocEntries(getSourceText());}function findEntryByPosition(position){const rawEntries=getRawEntries();return rawEntries.find(entry=>position>=entry.start&&position<=entry.end)||rawEntries.find(entry=>position<entry.start)||rawEntries[rawEntries.length-1]||null;}function getCurrentContextEntry(){if(editor){const pos=editor.selectionStart??0;return findEntryByPosition(pos);}const visibleBlock=Array.from(txtBody?.querySelectorAll('[data-block-index]')||[]).find(node=>{const rect=node.getBoundingClientRect();return rect.top<=window.innerHeight*0.35&&rect.bottom>=window.innerHeight*0.2;});if(visibleBlock){const sourceIndex=Number(visibleBlock.getAttribute('data-block-index'));const rawEntries=getRawEntries();return rawEntries.find(entry=>entry.sourceIndex===sourceIndex)||rawEntries[0]||null;}return getRawEntries()[0]||null;}function clipLabel(label){return label.length>34?label.slice(0,34)+'…':label;}function setActiveTocEntry(entryId){activeTocEntryId=typeof entryId==='string'?entryId:'';tocList?.querySelectorAll('[data-toc-id]').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.tocId===activeTocEntryId));}function renderToc(){if(!tocList||!tocEmpty)return;tocEmpty.style.display=tocEntries.length?'none':'block';tocList.innerHTML=tocEntries.map((entry,listIndex)=>'<button type=\"button\" class=\"toc-entry'+(entry.id===activeTocEntryId?' is-active':'')+'\" data-toc-id=\"'+escapeHtml(entry.id)+'\"><span class=\"toc-entry-title\">'+escapeHtml(clipLabel(entry.label))+'</span><span class=\"toc-entry-meta\">'+(entry.kind==='manual'?'手動':'自動')+' ・ '+(entry.lineCount||1)+' 行</span></button>').join('');tocList.querySelectorAll('[data-toc-id]').forEach(btn=>btn.addEventListener('click',()=>navigateToTocEntry(btn.dataset.tocId||'')));}function openToc(){if(!tocModal)return;tocModal.classList.add('on');tocModal.setAttribute('aria-hidden','false');tocCloseBtn?.focus();}function closeToc(){if(!tocModal)return;tocModal.classList.remove('on');tocModal.setAttribute('aria-hidden','true');}function centerEditorSelection(position){if(!editor)return;const textBefore=(editor.value||'').slice(0,Math.max(0,position));const style=window.getComputedStyle(editor);const lineHeight=parseFloat(style.lineHeight)||parseFloat(style.fontSize)*1.8||28;const lineCount=textBefore.split('\\n').length-1;const lineOffset=lineCount*lineHeight;const editorTarget=lineOffset-editor.clientHeight/2+lineHeight;editor.scrollTop=Math.max(0,Math.min(editorTarget,editor.scrollHeight-editor.clientHeight));const rect=editor.getBoundingClientRect();const pageTarget=window.scrollY+rect.top+lineOffset-window.innerHeight/2+lineHeight;const maxPageScroll=Math.max(0,document.documentElement.scrollHeight-window.innerHeight);window.scrollTo({top:Math.max(0,Math.min(pageTarget,maxPageScroll)),behavior:'smooth'});}function navigateToTocEntry(entryId){const entry=tocEntries.find(item=>item.id===entryId);if(!entry)return;setActiveTocEntry(entry.id);if(editor){editor.focus();editor.setSelectionRange(entry.start,entry.start,'forward');centerEditorSelection(entry.start);}else{const target=document.querySelector('[data-block-index=\"'+entry.sourceIndex+'\"]')||document.getElementById('txt-block-'+entry.sourceIndex);target?.scrollIntoView({behavior:'smooth',block:'start'});}closeToc();}function rebuildToc(){const autoEntries=buildAutoTocEntries(getSourceText());tocEntries=[...manualTocEntries,...autoEntries];if(!tocEntries.some(entry=>entry.id===activeTocEntryId))activeTocEntryId=tocEntries[0]?.id||'';renderToc();if(tocBtn)tocBtn.style.display=tocEntries.length?'inline-flex':'none';}function addManualTocEntry(){const baseEntry=getCurrentContextEntry();if(!baseEntry){window.alert('目前找不到可加入的段落。');return;}const suggested=baseEntry.label||'未命名段落';const label=(window.prompt('新增目錄名稱：',suggested)||'').trim();if(!label)return;manualTocEntries.push({...baseEntry,id:'manual-'+Date.now()+'-'+manualTocEntries.length,kind:'manual',label,lineCount:baseEntry.lineCount||1});rebuildToc();setActiveTocEntry(manualTocEntries[manualTocEntries.length-1]?.id||'');}function makeWindowDraggable(windowEl,handleEl){if(!windowEl||!handleEl)return;let drag=null;const onMove=event=>{if(!drag)return;const nextLeft=Math.min(Math.max(12,event.clientX-drag.offsetX),window.innerWidth-windowEl.offsetWidth-12);const nextTop=Math.min(Math.max(12,event.clientY-drag.offsetY),window.innerHeight-windowEl.offsetHeight-12);windowEl.style.left=nextLeft+'px';windowEl.style.top=nextTop+'px';windowEl.style.right='auto';windowEl.classList.add('dragging');};const onUp=()=>{drag=null;windowEl.classList.remove('dragging');window.removeEventListener('pointermove',onMove);window.removeEventListener('pointerup',onUp);};handleEl.addEventListener('pointerdown',event=>{if(event.target.closest('button'))return;const rect=windowEl.getBoundingClientRect();drag={offsetX:event.clientX-rect.left,offsetY:event.clientY-rect.top};windowEl.style.left=rect.left+'px';windowEl.style.top=rect.top+'px';windowEl.style.right='auto';window.addEventListener('pointermove',onMove);window.addEventListener('pointerup',onUp);});}tocBtn?.addEventListener('click',openToc);tocModalBg?.addEventListener('click',closeToc);tocCloseBtn?.addEventListener('click',closeToc);tocRefreshBtn?.addEventListener('click',()=>rebuildToc());tocAddBtn?.addEventListener('click',addManualTocEntry);document.addEventListener('keydown',event=>{const key=event.key.toLowerCase();if(event.key==='Escape'&&tocModal?.classList.contains('on')){event.preventDefault();closeToc();return;}if(event.ctrlKey&&!event.shiftKey&&!event.altKey&&!event.metaKey&&key==='j'&&tocEntries.length){event.preventDefault();openToc();}});editor?.addEventListener('input',rebuildToc);makeWindowDraggable(tocWindow,tocDragHandle);if(txtBody&&'IntersectionObserver'in window){const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(!visible)return;const sourceIndex=Number(visible.target.getAttribute('data-block-index'));const matched=tocEntries.find(entry=>entry.sourceIndex===sourceIndex&&entry.kind==='auto')||tocEntries.find(entry=>entry.sourceIndex===sourceIndex);if(matched)setActiveTocEntry(matched.id);},{root:null,rootMargin:'-20% 0px -60% 0px',threshold:[0.15,0.35,0.55]});txtBody.querySelectorAll('[data-block-index]').forEach(node=>observer.observe(node));}window.refreshTxtToc=rebuildToc;rebuildToc();})();</script>` : ''}
