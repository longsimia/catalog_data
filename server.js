...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    renderHistoryButton();updateUndoButtons();loadHistoryVersions();window.setInterval(()=>{if(hasUnsavedChanges())saveText('auto');},AUTOSAVE_MS);window.addEventListener('beforeunload',handleBeforeUnload);history.pushState({txtPreviewGuard:true},'');window.addEventListener('popstate',()=>{if(allowLeave||!hasUnsavedChanges())return;history.pushState({txtPreviewGuard:true},'');showLeaveModal(true);});
  </script>` : ''}
  ${noContextMenuScript}
</body>
</html>`;
