...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function updateFindSummary(activeIndex=-1){if(!findSummary)return;const needle=String(findInput?.value||'');if(!needle){findSummary.textContent='請先輸入要尋找的文字。';return;}const total=countMatches(editor?.value||'',needle);if(!total){findSummary.textContent='目前找不到符合結果。';return;}if(activeIndex>=0){findSummary.textContent='共 '+total+' 筆符合結果，目前選到第 '+getMatchOrdinal(editor?.value||'',needle,activeIndex)+' 筆。';return;}findSummary.textContent='共 '+total+' 筆符合結果。';}
