...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .floating-window{position:fixed;top:96px;right:40px;left:auto;transform:none;z-index:80;width:min(88vw,280px);border:1px solid var(--line);border-radius:14px;background:var(--bg);box-shadow:0 18px 48px rgba(0,0,0,.18);pointer-events:auto;overflow:hidden}
    ...
    .history-layout{display:grid;grid-template-columns:minmax(0,1fr);gap:16px;margin-top:18px}
    ...
    .history-mobile-diff{display:none;gap:14px}
    .history-mobile-block{display:grid;gap:10px}
    .history-mobile-line{display:grid;grid-template-columns:28px minmax(0,1fr);gap:12px;align-items:flex-start;padding:0;font-size:16px;line-height:1.88;word-break:break-word}
    .history-mobile-sign{font-size:24px;line-height:1;color:currentColor;text-align:center;padding-top:4px}
    .history-mobile-text{white-space:pre-wrap}
    .history-mobile-line.plus{color:#2c8d55}
    .history-mobile-line.minus{color:#bf4a5a}
    .history-mobile-line.same{grid-template-columns:minmax(0,1fr);color:var(--text)}
    .history-mobile-line.same .history-mobile-text{padding-left:40px}
    .history-mobile-ellipsis{padding-left:40px;color:var(--muted);font-size:26px;line-height:1.1;letter-spacing:.18em}
    .history-desktop-diff{display:grid}
    ...
    @media (max-width: 900px){.history-diff{grid-template-columns:1fr}}
    @media (max-width: 720px){.history-detail-modal{align-items:stretch;justify-content:stretch}.history-detail-modal .modal-bg{display:none}.history-detail-modal-card{width:100vw;max-height:100dvh;border:none;border-radius:0}.history-detail-shell{max-height:100dvh}.history-detail-topbar{padding:18px 18px 16px}.history-detail-body{padding:16px 16px 24px}.history-detail-footer{padding:14px 16px calc(18px + env(safe-area-inset-bottom))}.history-meta{display:grid;gap:6px;align-items:start}.history-desktop-diff{display:none}.history-mobile-diff{display:grid}}
...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderDiffHtml(sourceText,targetText){const left=normalizeLineEndings(sourceText).split('\\n');const right=normalizeLineEndings(targetText).split('\\n');const size=Math.max(left.length,right.length);function renderLineDiff(a,b){const source=String(a??'');const target=String(b??'');if(source===target)return{source:'<span class="diff-line">'+(source?escapeHtml(source):'&nbsp;')+'</span>',target:'<span class="diff-line">'+(target?escapeHtml(target):'&nbsp;')+'</span>'};let start=0;const maxPrefix=Math.min(source.length,target.length);while(start<maxPrefix&&source[start]===target[start])start+=1;let endSource=source.length-1,endTarget=target.length-1;while(endSource>=start&&endTarget>=start&&source[endSource]===target[endTarget]){endSource-=1;endTarget-=1;}const sourcePrefix=source.slice(0,start);const sourceChanged=source.slice(start,endSource+1);const sourceSuffix=source.slice(endSource+1);const targetPrefix=target.slice(0,start);const targetChanged=target.slice(start,endTarget+1);const targetSuffix=target.slice(endTarget+1);const renderChanged=value=>value?'<mark class="diff-mark">'+escapeHtml(value)+'</mark>':'';const renderLine=(prefix,changed,suffix,fallback='&nbsp;')=>{const html=escapeHtml(prefix)+renderChanged(changed)+escapeHtml(suffix);return '<span class="diff-line">'+(html||fallback)+'</span>';};return{source:renderLine(sourcePrefix,sourceChanged,sourceSuffix),target:renderLine(targetPrefix,targetChanged,targetSuffix)};}const sourceRows=[];const targetRows=[];for(let i=0;i<size;i+=1){const row=renderLineDiff(left[i]??'',right[i]??'');sourceRows.push(row.source);targetRows.push(row.target);}return{source:sourceRows.join('')||'<div class="diff-empty">沒有內容</div>',target:targetRows.join('')||'<div class="diff-empty">沒有內容</div>'};}
    function toHistoryParagraphs(text){const lines=normalizeLineEndings(text).split('\\n');const paragraphs=[];let current=[];const flush=()=>{if(!current.length)return;const raw=current.join(' ').replace(/\\s+/g,' ').trim();if(raw)paragraphs.push(raw);current=[];};for(const line of lines){if(!String(line||'').trim()){flush();continue;}current.push(String(line||'').trim());}flush();return paragraphs;}
    function renderHistoryMobileDiff(sourceText,targetText){const left=toHistoryParagraphs(sourceText);const right=toHistoryParagraphs(targetText);const size=Math.max(left.length,right.length);const changedIndices=[];for(let i=0;i<size;i+=1){if((left[i]??'')!==(right[i]??''))changedIndices.push(i);}if(!changedIndices.length){return'<div class="history-mobile-block"><div class="history-mobile-line same"><div class="history-mobile-text">'+escapeHtml(right.join('\\n\\n')||left.join('\\n\\n')||'沒有內容')+'</div></div></div>';}const visible=new Set();changedIndices.forEach(idx=>{for(let cursor=Math.max(0,idx-1);cursor<=Math.min(size-1,idx+1);cursor+=1)visible.add(cursor);});const parts=[];let collapsed=false;for(let i=0;i<size;i+=1){const leftLine=String(left[i]??'');const rightLine=String(right[i]??'');if(!visible.has(i)){if(!collapsed){parts.push('<div class="history-mobile-ellipsis">...</div>');collapsed=true;}continue;}collapsed=false;if(leftLine===rightLine){parts.push('<div class="history-mobile-line same"><div class="history-mobile-text">'+escapeHtml(leftLine||'沒有內容')+'</div></div>');continue;}if(leftLine&&leftLine!==rightLine){parts.push('<div class="history-mobile-line plus"><div class="history-mobile-sign">+</div><div class="history-mobile-text">'+escapeHtml(leftLine)+'</div></div>');}if(rightLine&&leftLine!==rightLine){parts.push('<div class="history-mobile-line minus"><div class="history-mobile-sign">-</div><div class="history-mobile-text">'+escapeHtml(rightLine)+'</div></div>');}}return'<div class="history-mobile-block">'+parts.join('')+'</div>';}
    function openHistoryDetail(versionId){selectedHistoryId=versionId||selectedHistoryId;renderHistoryList();renderHistoryDiff();openModal(historyDetailModal);historyDetailCloseBtn?.focus();}
    ...
    function renderHistoryDiff(){const version=historyVersions.find(entry=>entry.id===selectedHistoryId)||historyVersions[historyVersions.length-1]||null;if(!version){if(historyVersionTitle)historyVersionTitle.textContent='尚未選擇版本';if(historyVersionMeta)historyVersionMeta.textContent='';if(historyVersionDiff)historyVersionDiff.innerHTML='<div class="diff-empty">沒有可比較的編輯記錄。</div>';if(historyCurrentDiff)historyCurrentDiff.innerHTML='<div class="diff-empty">沒有可比較的目前內容。</div>';if(historyMobileDiff)historyMobileDiff.innerHTML='<div class="diff-empty">沒有可比較的編輯記錄。</div>';if(historyRestoreBtn)historyRestoreBtn.disabled=true;return;}selectedHistoryId=version.id;if(historyVersionTitle)historyVersionTitle.textContent=version.filename||'未命名 TXT';if(historyVersionMeta)historyVersionMeta.textContent=[getHistorySavedAtLabel(version),formatHistoryMode(version.mode),version.savedBy||''].filter(Boolean).join(' ・ ');const diff=renderDiffHtml(version.text||'',editor?.value||'');if(historyVersionDiff)historyVersionDiff.innerHTML=diff.source;if(historyCurrentDiff)historyCurrentDiff.innerHTML=diff.target;if(historyMobileDiff)historyMobileDiff.innerHTML=renderHistoryMobileDiff(version.text||'',editor?.value||'');if(historyRestoreBtn)historyRestoreBtn.disabled=false;}
