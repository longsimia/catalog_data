...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderHighlightedHistoryHtml(sourceText,targetText){const left=splitHistoryParagraphs(sourceText);const right=splitHistoryParagraphs(targetText);const rows=Array.from({length:left.length+1},()=>Array(right.length+1).fill(0));for(let i=left.length-1;i>=0;i-=1){for(let j=right.length-1;j>=0;j-=1){rows[i][j]=left[i]===right[j]?rows[i+1][j+1]+1:Math.max(rows[i+1][j],rows[i][j+1]);}}const matched=new Set();let i=0,j=0;while(i<left.length&&j<right.length){if(left[i]===right[j]){matched.add(i);i+=1;j+=1;continue;}if(rows[i+1][j]>=rows[i][j+1])i+=1;else j+=1;}const parts=[];let collapsed=false;for(let idx=0;idx<left.length;idx+=1){const line=left[idx]||'';if(matched.has(idx)){if(String(line).trim()){if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}}else if(parts.length&&parts[parts.length-1]!==''&&parts[parts.length-1]!=='<span class="history-ellipsis">…</span>'){parts.push('');}continue;}collapsed=false;if(!String(line).trim()){parts.push('');continue;}parts.push('<span class="diff-line"><mark class="diff-mark">'+escapeHtml(line)+'</mark></span>');}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}
