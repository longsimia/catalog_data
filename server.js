...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function buildLcsMatchedIndices(left,right){const rows=Array.from({length:left.length+1},()=>Array(right.length+1).fill(0));for(let i=left.length-1;i>=0;i-=1){for(let j=right.length-1;j>=0;j-=1){rows[i][j]=left[i]===right[j]?rows[i+1][j+1]+1:Math.max(rows[i+1][j],rows[i][j+1]);}}const leftMatched=new Set(),rightMatched=new Set();let i=0,j=0;while(i<left.length&&j<right.length){if(left[i]===right[j]){leftMatched.add(i);rightMatched.add(j);i+=1;j+=1;continue;}if(rows[i+1][j]>=rows[i][j+1])i+=1;else j+=1;}return{leftMatched,rightMatched};}
    ...
    function renderHistoryBlock(sourceBlock,sign=''){const lines=String(sourceBlock??'').split('\\n');if(sign==='+'||sign==='-'){return lines.map((line,index)=>'<span class="diff-line diff-line-with-sign"><span class="diff-sign">'+(index===0?sign:'')+'</span><mark class="diff-mark">'+escapeHtml(line||' ')+'</mark></span>').join('');}return lines.map(line=>'<span class="diff-line"><mark class="diff-mark">'+escapeHtml(line||' ')+'</mark></span>').join('');}
    function renderHighlightedHistoryHtml(sourceText,targetText){const left=splitHistoryBlocks(sourceText);const right=splitHistoryBlocks(targetText);const{leftMatched,rightMatched}=buildLcsMatchedIndices(left,right);const parts=[];let collapsed=false;let rightCursor=0;for(let idx=0;idx<left.length;idx+=1){if(leftMatched.has(idx)){while(rightCursor<right.length&&!rightMatched.has(rightCursor))rightCursor+=1;rightCursor+=1;if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}continue;}collapsed=false;let compareBlock='';while(rightCursor<right.length&&!rightMatched.has(rightCursor)){compareBlock=right[rightCursor]||'';rightCursor+=1;break;}parts.push(renderHistoryBlock(left[idx],getHistoryBlockChangeSign(left[idx],compareBlock)));}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}
    function buildHistoryCompareContext(sourceText,targetText){const left=splitHistoryBlocks(sourceText);const right=splitHistoryBlocks(targetText);const{leftMatched,rightMatched}=buildLcsMatchedIndices(left,right);const leftVisible=new Set(),rightVisible=new Set();for(let idx=0;idx<left.length;idx+=1){if(leftMatched.has(idx))continue;for(let pointer=Math.max(0,idx-1);pointer<=Math.min(left.length-1,idx+1);pointer+=1)leftVisible.add(pointer);}for(let idx=0;idx<right.length;idx+=1){if(rightMatched.has(idx))continue;for(let pointer=Math.max(0,idx-1);pointer<=Math.min(right.length-1,idx+1);pointer+=1)rightVisible.add(pointer);}if(!leftVisible.size&&!rightVisible.size){return{leftHtml:'<div class="diff-empty">此版本與目前內文相同。</div>',rightHtml:'<div class="diff-empty">此版本與目前內文相同。</div>'};}function renderBlocks(blocks,visible,matched){const parts=[];let collapsed=false;for(let idx=0;idx<blocks.length;idx+=1){if(!visible.has(idx)){if(!collapsed){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}continue;}collapsed=false;const block=blocks[idx]||'';const lines=block.split('\\n');const highlighted=!matched.has(idx);parts.push(lines.map(line=>highlighted?'<span class="diff-line"><mark class="diff-mark">'+escapeHtml(line||' ')+'</mark></span>':'<span class="diff-line">'+escapeHtml(line||' ')+'</span>').join(''));}const html=parts.join('');return html.trim()?html:'<div class="diff-empty">沒有內容</div>';}return{leftHtml:renderBlocks(left,leftVisible,leftMatched),rightHtml:renderBlocks(right,rightVisible,rightMatched)};}
