...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .toc-list{display:grid;gap:8px;max-height:min(62dvh,520px);overflow:auto;padding-right:2px}
    .toc-entry{width:100%;border:1px solid transparent;border-radius:12px;background:transparent;color:var(--text);text-align:left;padding:10px 12px;cursor:pointer;transition:background .18s ease,border-color .18s ease,color .18s ease}
    .toc-entry:hover,.toc-entry.is-active{background:var(--panel);border-color:var(--line)}
    .toc-entry-title{display:block;font-size:14px;line-height:1.6;word-break:break-word}
    .toc-entry-meta{display:block;margin-top:4px;color:var(--muted);font-size:12px;line-height:1.45}
...
<body>
      ...
      <div class="meta-head">
        ...
        <div class="meta-side">
          <div class="actions">
            ...
            ${isTxt ? `<button type="button" class="action-btn" id="tocBtn" title="段落導覽">導覽</button>` : ''}
  ...
  ${isTxt ? `<div class="modal floating-modal" id="tocModal" aria-hidden="true"><div class="modal-bg" id="tocModalBg"></div><section class="floating-window toc-window" id="tocWindow" role="dialog" aria-labelledby="tocModalTitle"><div class="window-head"><h3 id="tocModalTitle">段落導覽</h3><button type="button" class="window-close" id="tocCloseBtn" aria-label="關閉">×</button></div><div class="window-body"><div id="tocEmpty" class="toc-empty" style="display:none">目前沒有可導覽的段落。</div><div id="tocList" class="toc-list"></div></div></section></div>` : ''}
  ${isTxt && canEditTxt ? `<div class="modal floating-modal" id="formatModal" aria-hidden="true"><div class="modal-bg" id="formatModalBg"></div><section class="floating-window format-modal-card" id="formatWindow" role="dialog" aria-labelledby="formatModalTitle"><div class="window-head" id="formatDragHandle"><h3 id="formatModalTitle">格式化排版</h3><button type="button" class="window-close" id="formatCloseBtn" aria-label="關閉">×</button></div><div class="window-body"><div class="encoding-field"><label for="encodingToggleBtn">切換編碼</label><div class="encoding-menu-wrap"><button type="button" id="encodingToggleBtn" class="encoding-toggle" aria-haspopup="true" aria-expanded="false" aria-controls="encodingMenu"><span id="encodingToggleLabel">${escapeXml(getEncodingToggleLabel(selectedEncoding))}</span><span class="encoding-toggle-caret" aria-hidden="true">▾</span></button><select id="encodingSelect" class="encoding-select" aria-label="TXT 編碼" tabindex="-1">${encodingOptionsHtml}</select><div id="encodingMenu" class="encoding-menu" role="menu">${encodingMenuHtml}</div></div></div><div class="format-grid"><button type="button" class="format-action-btn format-action-btn-wide" data-action="find_replace">尋找與取代</button><button type="button" class="format-action-btn" data-action="indent_add">插入段首縮排</button><button type="button" class="format-action-btn" data-action="indent_remove">去除段首縮排</button><button type="button" class="format-action-btn" data-action="keep_blank_line">在段落與段落間保留一個空行</button><button type="button" class="format-action-btn" data-action="trim_blank_lines">去除段落與段落間所有的空行</button><button type="button" class="format-action-btn" data-action="punct_fullwidth">半形符號轉全形符號</button><button type="button" class="format-action-btn" data-action="cn_to_twp">简体中文 轉 繁體中文</button><button type="button" class="format-action-btn" data-action="t_to_cn">繁體中文 轉 简体中文</button><button type="button" class="format-action-btn" data-action="cjk_spacing">在中英文之間插入空白</button></div></div></section></div>` : ''}
  ...
  ${isTxt ? `<script>(()=>{const tocBtn=document.getElementById('tocBtn'),tocModal=document.getElementById('tocModal'),tocModalBg=document.getElementById('tocModalBg'),tocCloseBtn=document.getElementById('tocCloseBtn'),tocList=document.getElementById('tocList'),tocEmpty=document.getElementById('tocEmpty'),editor=document.getElementById('editor'),txtBody=document.getElementById('txtBody');let tocEntries=[],activeTocIndex=-1;function normalizeTocText(text){return String(text||'').replace(/\\r\\n/g,'\\n').replace(/\\r/g,'\\n');}function buildRawTocEntries(text){const normalized=normalizeTocText(text);const lines=normalized.split('\\n');const entries=[];let offset=0,start=-1,entryLines=[],end=-1;function pushEntry(){if(start<0||!entryLines.length)return;const firstLine=(entryLines.find(line=>line.trim())||'').replace(/\\s+/g,' ').trim();entries.push({index:entries.length,start,end,label:firstLine||('第 '+(entries.length+1)+' 段'),lineCount:entryLines.length,text:entryLines.join('\\n')});start=-1;entryLines=[];end=-1;}for(let idx=0;idx<lines.length;idx+=1){const line=lines[idx],lineStart=offset,hasBreak=idx<lines.length-1;offset+=line.length+(hasBreak?1:0);if(line.trim()){if(start<0)start=lineStart;entryLines.push(line);end=offset-(hasBreak?1:0);continue;}pushEntry();}pushEntry();return entries;}function isExcludedLabel(label){const text=String(label||'').trim();if(!text)return true;if(/^[【《「〔\\(（][^【《「〔\\(（\\]】》」〕\\)）]*[】》」〕\\)）]$/.test(text))return true;if(/^[\\s　]*[【《「〔\\(（]/.test(text))return true;return false;}function isSceneLikeLabel(label){const text=String(label||'').trim();if(!text||isExcludedLabel(text))return false;const solidSymRe=/^[\\s　]*[■□●○◆◇★☆▲△▼▽▶▷◀◁◉◈◦•※✦✧✩✪✫✬✭✮✯✰◐◑◒◓◔◕⬛⬜🔲🔳▪▫◎〇]/u;const dateRe=/\\d{1,2}[\\s　]*[月\\/\\-.]\\s*\\d{1,2}|\\d{4}[\\/\\-.]\\d{1,2}[\\/\\-.]\\d{1,2}|星期[一二三四五六七日天]|週[一二三四五六日天]|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|Mon\\.?|Tue\\.?|Wed\\.?|Thu\\.?|Fri\\.?|Sat\\.?|Sun\\.?|(?:^|\\s)(?:[AP]M)(?:\\s|$)|午前|午後|深夜|早朝|夜明け/iu;return solidSymRe.test(text)||dateRe.test(text);}function buildTocEntries(text){const rawEntries=buildRawTocEntries(text);const strictEntries=rawEntries.filter(entry=>isSceneLikeLabel(entry.label));if(strictEntries.length)return strictEntries.map((entry,index)=>({...entry,index}));const fallbackEntries=rawEntries.filter(entry=>!isExcludedLabel(entry.label)&&entry.label.replace(/[\\s　]/g,'').length>=2);return fallbackEntries.map((entry,index)=>({...entry,index}));}function clipLabel(label){return label.length>34?label.slice(0,34)+'…':label;}function setActiveTocIndex(index){activeTocIndex=Number.isInteger(index)?index:-1;tocList?.querySelectorAll('[data-toc-index]').forEach(btn=>btn.classList.toggle('is-active',Number(btn.dataset.tocIndex)===activeTocIndex));}function renderToc(){if(!tocList||!tocEmpty)return;tocEmpty.style.display=tocEntries.length?'none':'block';tocList.innerHTML=tocEntries.map(entry=>'<button type="button" class="toc-entry'+(entry.index===activeTocIndex?' is-active':'')+'\" data-toc-index=\"'+entry.index+'\"><span class=\"toc-entry-title\">'+escapeHtml(clipLabel(entry.label))+'</span><span class=\"toc-entry-meta\">第 '+(entry.index+1)+' 項 ・ '+entry.lineCount+' 行</span></button>').join('');tocList.querySelectorAll('[data-toc-index]').forEach(btn=>btn.addEventListener('click',()=>navigateToTocEntry(Number(btn.dataset.tocIndex))));}function openToc(){if(!tocModal)return;tocModal.classList.add('on');tocModal.setAttribute('aria-hidden','false');tocCloseBtn?.focus();}function closeToc(){if(!tocModal)return;tocModal.classList.remove('on');tocModal.setAttribute('aria-hidden','true');}function centerEditorSelection(position){if(!editor)return;const textBefore=(editor.value||'').slice(0,Math.max(0,position));const style=window.getComputedStyle(editor);const lineHeight=parseFloat(style.lineHeight)||parseFloat(style.fontSize)*1.8||28;const lineCount=textBefore.split('\\n').length-1;const lineOffset=lineCount*lineHeight;const editorTarget=lineOffset-editor.clientHeight/2+lineHeight;editor.scrollTop=Math.max(0,Math.min(editorTarget,editor.scrollHeight-editor.clientHeight));const rect=editor.getBoundingClientRect();const pageTarget=window.scrollY+rect.top+lineOffset-window.innerHeight/2+lineHeight;const maxPageScroll=Math.max(0,document.documentElement.scrollHeight-window.innerHeight);window.scrollTo({top:Math.max(0,Math.min(pageTarget,maxPageScroll)),behavior:'smooth'});}function navigateToTocEntry(index){const entry=tocEntries.find(item=>item.index===index);if(!entry)return;setActiveTocIndex(index);if(editor){editor.focus();editor.setSelectionRange(entry.start,entry.start,'forward');centerEditorSelection(entry.start);}else{const target=document.querySelector('[data-block-index=\"'+entry.index+'\"]')||document.getElementById('txt-block-'+entry.index);target?.scrollIntoView({behavior:'smooth',block:'start'});}closeToc();}function refreshToc(){const sourceText=editor?editor.value:(txtBody?Array.from(txtBody.querySelectorAll('[data-block-index]')).map(node=>node.textContent||'').join('\\n\\n'):'');tocEntries=buildTocEntries(sourceText);if(activeTocIndex>=tocEntries.length)activeTocIndex=tocEntries.length?tocEntries[tocEntries.length-1].index:-1;if(activeTocIndex<0&&tocEntries.length)activeTocIndex=0;renderToc();if(tocBtn)tocBtn.style.display=tocEntries.length?'inline-flex':'none';}tocBtn?.addEventListener('click',openToc);tocModalBg?.addEventListener('click',closeToc);tocCloseBtn?.addEventListener('click',closeToc);document.addEventListener('keydown',event=>{const key=event.key.toLowerCase();if(event.key==='Escape'&&tocModal?.classList.contains('on')){event.preventDefault();closeToc();return;}if(event.ctrlKey&&!event.shiftKey&&!event.altKey&&!event.metaKey&&key==='j'&&tocEntries.length){event.preventDefault();openToc();}});editor?.addEventListener('input',refreshToc);if(txtBody&&'IntersectionObserver'in window){const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(!visible)return;const nextIndex=Number(visible.target.getAttribute('data-block-index'));if(Number.isFinite(nextIndex))setActiveTocIndex(nextIndex);},{root:null,rootMargin:'-20% 0px -60% 0px',threshold:[0.15,0.35,0.55]});txtBody.querySelectorAll('[data-block-index]').forEach(node=>observer.observe(node));}window.refreshTxtToc=refreshToc;refreshToc();})();</script>` : ''}
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function applyState(state,{focusEditor=true}={}){clearInputGroupTimer();inputGroupState=null;inputGroupTarget='';pendingInputState=null;suspendTracking=true;if(editor)editor.value=state?.text||'';if(filenameInput)filenameInput.value=state?.filename||'';resizeEditor();window.refreshTxtToc?.();suspendTracking=false;syncDirtyState();updateUndoButtons();updateFindSummary();if(focusEditor)editor?.focus();}
    ...
    function centerSelectionInEditor(index){if(!editor)return;const textBefore=(editor.value||'').slice(0,Math.max(0,index));const style=window.getComputedStyle(editor);const lineHeight=parseFloat(style.lineHeight)||parseFloat(style.fontSize)*1.8||28;const lineCount=textBefore.split('\\n').length-1;const lineOffset=lineCount*lineHeight;const editorTarget=lineOffset-editor.clientHeight/2+lineHeight;editor.scrollTop=Math.max(0,Math.min(editorTarget,editor.scrollHeight-editor.clientHeight));const rect=editor.getBoundingClientRect();const pageTarget=window.scrollY+rect.top+lineOffset-window.innerHeight/2+lineHeight;const maxPageScroll=Math.max(0,document.documentElement.scrollHeight-window.innerHeight);window.scrollTo({top:Math.max(0,Math.min(pageTarget,maxPageScroll)),behavior:'smooth'});}
