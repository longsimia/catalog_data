...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderHistoryList(){if(!historyList)return;historyList.innerHTML=historyVersions.map(version=>'<button type="button" class="history-entry'+(version.id===selectedHistoryId?' active':'')+'" data-history-id="'+escapeHtml(version.id)+'"><div>'+escapeHtml(version.filename||'未命名 TXT')+'</div><small>'+escapeHtml(getHistorySavedAtLabel(version))+' ・ '+escapeHtml(formatHistoryMode(version.mode))+(version.savedBy?' ・ '+escapeHtml(version.savedBy):'')+'</small></button>').join('');historyList.querySelectorAll('[data-history-id]').forEach(btn=>btn.addEventListener('click',()=>selectHistoryVersion(btn.getAttribute('data-history-id')||'')));}
    ...
    async function clearHistoryVersions(){if(!historyUrl)return;const confirmed=await openHistoryClearConfirm();if(!confirmed)return;try{const res=await fetch(historyUrl,{method:'DELETE',headers:{'Authorization':'Bearer '+(localStorage.getItem('adm-token')||'')}});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'清空編輯記錄失敗');historyVersions=[];selectedHistoryId='';renderHistoryButton();closeHistoryModal();setStatusMessage('已清空編輯記錄。','success');}catch(err){setStatusMessage(err.message||'清空編輯記錄失敗','error');}}
    ...
    document.addEventListener('keydown',event=>{const key=event.key.toLowerCase();if(event.ctrlKey&&!event.shiftKey&&!event.altKey&&!event.metaKey&&key==='h'){event.preventDefault();openFindReplaceModal();return;}if(event.ctrlKey&&!event.shiftKey&&!event.altKey&&!event.metaKey&&key==='s'){event.preventDefault();saveText('manual');return;}if(event.ctrlKey&&!event.shiftKey&&!event.altKey&&!event.metaKey&&key==='z'){event.preventDefault();undoAction();return;}if(event.ctrlKey&&event.shiftKey&&!event.altKey&&!event.metaKey&&key==='z'){event.preventDefault();redoAction();return;}if(event.key==='Escape'&&historyClearConfirmModal?.classList.contains('on')){event.preventDefault();closeHistoryClearConfirm(false);return;}if(event.key==='Escape'&&historyModal?.classList.contains('on')){event.preventDefault();closeHistoryModal();return;}if(event.key==='Escape'&&findReplaceModal?.classList.contains('on')){event.preventDefault();closeFindReplaceModal();return;}if(event.key==='Escape'&&formatModal?.classList.contains('on')){event.preventDefault();closeFormatModal();return;}if(event.key==='Escape'&&leaveModal?.classList.contains('on')){event.preventDefault();closeLeaveModal();}});
