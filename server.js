...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function renderHistoryMobileDiff(sourceText,targetText){const left=normalizeLineEndings(sourceText).split('\\n');const right=normalizeLineEndings(targetText).split('\\n');const size=Math.max(left.length,right.length);const changedIndices=[];for(let i=0;i<size;i+=1){if((left[i]??'')!==(right[i]??''))changedIndices.push(i);}if(!changedIndices.length){return'<div class="history-mobile-block"><div class="history-mobile-line same"><div class="history-mobile-text">'+(escapeHtml((right.join('\\n')||left.join('\\n')||'沒有內容'))||'沒有內容')+'</div></div></div>';}const visible=new Set();changedIndices.forEach(idx=>{for(let cursor=Math.max(0,idx-1);cursor<=Math.min(size-1,idx+1);cursor+=1)visible.add(cursor);});const parts=[];let collapsed=false;for(let i=0;i<size;i+=1){const leftLine=String(left[i]??'');const rightLine=String(right[i]??'');if(!visible.has(i)){if(!collapsed){parts.push('<div class="history-mobile-ellipsis">...</div>');collapsed=true;}continue;}collapsed=false;if(leftLine===rightLine){parts.push('<div class="history-mobile-line same"><div class="history-mobile-text">'+(leftLine?escapeHtml(leftLine):'&nbsp;')+'</div></div>');continue;}if(leftLine){parts.push('<div class="history-mobile-line plus"><div class="history-mobile-sign">+</div><div class="history-mobile-text">'+escapeHtml(leftLine)+'</div></div>');}if(rightLine){parts.push('<div class="history-mobile-line minus"><div class="history-mobile-sign">-</div><div class="history-mobile-text">'+escapeHtml(rightLine)+'</div></div>');}}return'<div class="history-mobile-block">'+parts.join('')+'</div>';}
