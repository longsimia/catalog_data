...
function renderTextPreviewPage(item, file, text, options = {}) {
  ...
  const encodingMenuHtml = isTxt ? encodingMenuGroups.map(group => `
    <div class="encoding-menu-group">
      ${group.label ? `<div class="encoding-menu-label">${escapeXml(group.label)}</div>` : ''}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .encoding-menu-wrap{position:relative;width:156px;max-width:100%}
    .encoding-toggle{display:block;width:100%;border:none;background:transparent;color:var(--muted);padding:0 16px 0 0;font:inherit;font-size:12px;line-height:1.35;text-align:left;cursor:pointer;position:relative}
    .encoding-toggle:hover,.encoding-toggle[aria-expanded="true"]{color:var(--text)}
    .encoding-toggle-caret{position:absolute;right:0;top:50%;transform:translateY(-48%);font-size:10px;line-height:1}
    .encoding-select{position:absolute;pointer-events:none;opacity:0;width:1px;height:1px;inset:auto}
    .encoding-menu{position:absolute;top:calc(100% + 6px);left:0;display:none;width:100%;padding:10px 12px;border:1px solid var(--line);border-radius:12px;background:var(--bg);box-shadow:0 18px 48px rgba(0,0,0,.18);z-index:40}
    .encoding-menu.is-open{display:block}
    .encoding-menu-group + .encoding-menu-group{margin-top:8px;padding-top:8px;border-top:1px solid var(--line)}
    .encoding-menu-label{margin:0 0 4px;color:var(--muted);font-size:11px;line-height:1.35}
...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function openEncodingMenu(){if(encodingMenu)encodingMenu.classList.add('is-open');if(encodingToggleBtn)encodingToggleBtn.setAttribute('aria-expanded','true');}
