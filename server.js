...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function splitHistoryParagraphs(text){const lines=normalizeLineEndings(text).split('\\n');const blocks=[];let current=[];const flush=()=>{if(!current.length)return;blocks.push(current.join('\\n'));current=[];};for(const line of lines){if(!String(line||'').trim()){flush();continue;}current.push(String(line??''));}flush();return blocks;}
    function renderHighlightedHistoryHtml(sourceText,targetText){const left=splitHistoryParagraphs(sourceText);const right=splitHistoryParagraphs(targetText);const size=Math.max(left.length,right.length);function renderLine(a,b){const source=String(a??'');const target=String(b??'');if(source===target)return'<span class="diff-line">'+(source?escapeHtml(source):'&nbsp;')+'</span>';let start=0;const maxPrefix=Math.min(source.length,target.length);while(start<maxPrefix&&source[start]===target[start])start+=1;let endSource=source.length-1,endTarget=target.length-1;while(endSource>=start&&endTarget>=start&&source[endSource]===target[endTarget]){endSource-=1;endTarget-=1;}const prefix=source.slice(0,start);const changed=source.slice(start,endSource+1);const suffix=source.slice(endSource+1);const changedHtml=changed?'<mark class="diff-mark">'+escapeHtml(changed)+'</mark>':'';const html=escapeHtml(prefix)+changedHtml+escapeHtml(suffix);return'<span class="diff-line">'+(html||'&nbsp;')+'</span>';}function renderBlock(sourceBlock,targetBlock){const leftLines=String(sourceBlock??'').split('\\n');const rightLines=String(targetBlock??'').split('\\n');const lineCount=Math.max(leftLines.length,rightLines.length);const rows=[];for(let i=0;i<lineCount;i+=1)rows.push(renderLine(leftLines[i]??'',rightLines[i]??''));return rows.join('');}const parts=[];let collapsed=false;for(let i=0;i<size;i+=1){const sourceBlock=left[i]??'';const targetBlock=right[i]??'';if(sourceBlock===targetBlock){if(!collapsed&&sourceBlock){parts.push('<span class="history-ellipsis">…</span>');collapsed=true;}continue;}collapsed=false;parts.push(renderBlock(sourceBlock,targetBlock));}return parts.join('')||'<div class="diff-empty">沒有內容</div>';}
    ...
    function renderHistoryDiff(){const version=historyVersions.find(entry=>entry.id===selectedHistoryId)||historyVersions[historyVersions.length-1]||null;if(!version){if(historyVersionTitle)historyVersionTitle.textContent='尚未選擇版本';if(historyVersionMeta)historyVersionMeta.textContent='';if(historyVersionDiff)historyVersionDiff.innerHTML='<div class="diff-empty">沒有可比較的編輯記錄。</div>';if(historyRestoreBtn)historyRestoreBtn.disabled=true;return;}selectedHistoryId=version.id;if(historyVersionTitle)historyVersionTitle.textContent=version.filename||'未命名 TXT';if(historyVersionMeta)historyVersionMeta.textContent=[getHistorySavedAtLabel(version),formatHistoryMode(version.mode),version.savedBy||''].filter(Boolean).join(' ・ ');if(historyVersionDiff)historyVersionDiff.innerHTML=renderHighlightedHistoryHtml(version.text||'',editor?.value||'');if(historyRestoreBtn)historyRestoreBtn.disabled=false;}
