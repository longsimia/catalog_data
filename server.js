...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .editor{display:block;width:100%;min-height:50vh;border:none;outline:none;resize:vertical;overflow:auto;background:transparent;color:inherit;padding:0}
...
<body>
  ...
  ${isTxt && canEditTxt ? `<script>
    ...
    function applyTrackedState(nextState,successMessage,unchangedMessage='目前內容沒有變化。'){const before=snapshotState();if(statesEqual(before,nextState)){setStatusMessage(unchangedMessage);return false;}rememberUndoState(before);applyState(nextState);setStatusMessage(successMessage,'success');return true;}
    function handleFieldBeforeInput(event){if(suspendTracking)return;if(event?.inputType==='historyUndo'||event?.inputType==='historyRedo')return;pendingInputState=snapshotState();}
    function handleEditorInput(){if(suspendTracking)return;const current=snapshotState();if(pendingInputState&&!statesEqual(pendingInputState,current))rememberUndoState(pendingInputState);pendingInputState=null;resizeEditor();syncDirtyState();updateUndoButtons();}
    function handleFilenameInput(){if(suspendTracking)return;const current=snapshotState();if(pendingInputState&&!statesEqual(pendingInputState,current))rememberUndoState(pendingInputState);pendingInputState=null;syncDirtyState();updateUndoButtons();}
    function undoAction(){if(!undoStack.length)return;pendingInputState=null;const current=snapshotState();const previous=undoStack.pop();redoStack.push(current);applyState(previous);setStatusMessage('已復原。','success');}
    function redoAction(){if(!redoStack.length)return;pendingInputState=null;const current=snapshotState();const next=redoStack.pop();undoStack.push(current);applyState(next);setStatusMessage('已重做。','success');}
    ...
    function openFindReplaceModal(){openModal(findReplaceModal);setTimeout(()=>findInput?.focus(),0);findInput?.select();}
    ...
    function centerSelectionInEditor(index){if(!editor)return;const ratio=index/Math.max(1,(editor.value||'').length);const target=editor.scrollHeight*ratio-editor.clientHeight/2;editor.scrollTop=Math.max(0,Math.min(target,editor.scrollHeight-editor.clientHeight));}
    ...
    function findNextMatch(startIndex=null){if(!editor)return false;const needle=String(findInput?.value||'');if(!needle){setStatusMessage('請先輸入要尋找的文字。','error');return false;}const haystack=editor.value||'';const total=countMatches(haystack,needle);if(!total){setStatusMessage('找不到符合的文字。','error');return false;}const from=startIndex===null?(editor.selectionEnd??0):startIndex;let idx=haystack.indexOf(needle,from);if(idx<0&&from>0)idx=haystack.indexOf(needle,0);if(idx<0){setStatusMessage('找不到符合的文字。','error');return false;}const ordinal=getMatchOrdinal(haystack,needle,idx);editor.focus();editor.selectionStart=idx;editor.selectionEnd=idx+needle.length;centerSelectionInEditor(idx);setStatusMessage('已找到第 '+ordinal+' / '+total+' 筆。','success');return true;}
    function replaceCurrentMatch(){if(!editor)return false;const needle=String(findInput?.value||''),replacement=String(replaceInput?.value||'');if(!needle){setStatusMessage('請先輸入要尋找的文字。','error');return false;}const selected=editor.value.slice(editor.selectionStart??0,editor.selectionEnd??0);if(selected!==needle&&!findNextMatch(editor.selectionEnd??0))return false;const start=editor.selectionStart??0,end=editor.selectionEnd??start;pendingInputState=null;rememberUndoState(snapshotState());editor.setRangeText(replacement,start,end,'end');resizeEditor();syncDirtyState();editor.focus();const remaining=countMatches(editor.value,needle);setStatusMessage('已取代 1 筆，剩餘 '+remaining+' 筆符合結果。','success');return true;}
    function replaceAllMatches(){if(!editor)return;const needle=String(findInput?.value||''),replacement=String(replaceInput?.value||'');if(!needle){setStatusMessage('請先輸入要尋找的文字。','error');return;}const total=countMatches(editor.value,needle);if(!total){setStatusMessage('找不到符合的文字。','error');return;}applyTrackedState({text:editor.value.split(needle).join(replacement),filename:filenameInput?.value||''},'已取代 '+total+' 筆結果。','目前沒有找到要取代的文字。');}
